...
def get_calibration_for_voltage(picks, voltage_key="1950"):
    """
    Find the 2-point calibration (a,b) for the measurement whose label contains `voltage_key`
    (e.g., "1950V" or "1950"). Uses picked peaks from picks CSV.

    Returns (a,b,label) or (None,None,None) if not found / not valid.
    """
    # picks is dict keyed by basename -> {label, mu_low, mu_high}
    for base, info in picks.items():
        lbl = info.get("label", "")
        if voltage_key in lbl:
            mu_low = info.get("mu_low", None)
            mu_high = info.get("mu_high", None)
...
def fit_gaussian_to_file(folder_path, file_name, out_dir, a1950=None, b1950=None, frac_height=0.2, min_width=20):
    ...
    fig, ax = plt.subplots()
    title = ax.set_title(r"14.4 keV $\gamma$-Transition Gaussian Fit", pad=20)
    ...
    ax.set_xlabel("Channel")
        # ---- Add a SECOND bottom x-axis: Energy [keV] using 1950V calibration ----
    if a1950 is not None and b1950 is not None:
        # forward: channel -> energy
        ch_to_E = lambda ch: a1950 * ch + b1950
        # inverse: energy -> channel
        E_to_ch = lambda E: (E - b1950) / a1950

        ax_energy = ax.secondary_xaxis('bottom', functions=(ch_to_E, E_to_ch))

        # Push the energy axis below the channel axis (axes coords; negative = below)
        ax_energy.set_location(-0.25)

        ax_energy.set_xlabel("Energy (keV)")

        # Give extra room so the lower axis labels/ticks don't get clipped
        ax.tick_params(axis='x', pad=2)          # channel axis ticks
        ax_energy.tick_params(axis='x', pad=2)   # energy axis ticks

    ax.set_ylabel("Counts")


    fig.canvas.draw()
    title_box = title.get_window_extent(fig.canvas.renderer)
    ...
    fig.savefig("results/14.4keV_gaussfit.pdf", backend="pgf")
...
def main():
    ...
    picks = load_picks_csv(picks_path)
    # --- 1950V calibration for secondary (energy) axis on Gaussian plot ---
    a1950, b1950, lbl1950 = get_calibration_for_voltage(picks, voltage_key="1950")
    if a1950 is None or b1950 is None:
        print("WARNING: No valid 1950V 2-point calibration found. Energy axis will be skipped.")

    fit_gaussian_to_file(data_dir, "14_4_keV_line.asc", out_dir, a1950=a1950, b1950=b1950)
